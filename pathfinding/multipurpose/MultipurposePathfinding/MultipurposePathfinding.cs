...
namespace MultipurposePathfinding
{
    ...
    public class Pathfinding : IPathfinding
    {
        ...
        public void IterateInitialization(double deltaTime)
        {
            if (_initialized)
                return;
            ...
            if (_positionsAndTerrainsForFirstPassInitialization.IsEmpty())
            {
                if (Config.DebugEnabled)
                    Console.WriteLine($"{DateTimeOffset.UtcNow.ToUnixTimeMilliseconds()} " +
                                      $"{nameof(Pathfinding)}.{nameof(IterateTerrainGraphFirstPassUpdate)} " +
                                      $"completed");
        ...
        private void IterateTerrainGraphFurtherPassUpdate()
        {
            if (_positionsByTerrainForFurtherInitialization.IsEmpty())
            {
                if (Config.DebugEnabled)
                    Console.WriteLine($"{DateTimeOffset.UtcNow.ToUnixTimeMilliseconds()} " +
                                      $"{nameof(Pathfinding)}.{nameof(IterateTerrainGraphFurtherPassUpdate)} " +
                                      $"completed");
        ...
        private void IterateDiagonalConnectionUpdate()
        {
            if (_pointIdsByPositionsForInitialization.IsEmpty())
            {
                if (Config.DebugEnabled)
                    Console.WriteLine($"{DateTimeOffset.UtcNow.ToUnixTimeMilliseconds()} " +
                                      $"{nameof(Pathfinding)}.{nameof(IterateDiagonalConnectionUpdate)} " +
                                      $"completed");
