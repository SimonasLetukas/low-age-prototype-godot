...
public static class ShadowCast {
    ...
    private class OctantTransform(int xx, int xy, int yx, int yy)
    {
        ...
        public override string ToString() => $"[OctantTransform {Xx,2:D} {Xy,2:D} {Yx,2:D} {Yy,2:D}]";
    }
    private static readonly OctantTransform[] SOctantTransform =
    [
        new( 1,  0,  0,  1 ),   // 0 E-NE
        new( 0,  1,  1,  0 ),   // 1 NE-N
        new( 0, -1,  1,  0 ),   // 2 N-NW
    ...
    /// <param name="startingPosition">Starting position within the grid.</param>
    /// <param name="viewRadius">Maximum view distance; can be a fractional value.</param>
    public static void ComputeVisibility(IShadowCastGrid grid, Vector2Int startingPosition, float viewRadius)
    {
        // Viewer's cell is always visible.
        grid.SetLight(startingPosition.X, startingPosition.Y);
        ...
        foreach (var octantTransform in SOctantTransform)
        {
            CastLight(grid, startingPosition, viewRadius, 1, 1.0f, 0.0f, octantTransform);
        }
    }
    ...
    /// </summary>
    /// <param name="grid">The cell grid definition.</param>
    /// <param name="startingPosition">The starting position within the grid.</param>
    ...
    ///   the initial value.</param>
    /// <param name="octantTransform">Coordinate multipliers for the octant transform.</param>
    ///
    /// Maximum recursion depth is (Ceiling(viewRadius)).
    private static void CastLight(IShadowCastGrid grid, Vector2Int startingPosition, float viewRadius,
            int startColumn, float leftViewSlope, float rightViewSlope, OctantTransform octantTransform) 
    {
        ...
        // of the empty cell.
        var savedRightSlope = -1f;

        var xDim = grid.Size.X;
        var yDim = grid.Size.Y;
        ...
        for (var currentCol = startColumn; currentCol <= viewCeiling; currentCol++) 
        {
            ...
            for (var yc = currentCol; yc >= 0; yc--) 
            {
                ...
                var gridX = startingPosition.X + currentCol * octantTransform.Xx + yc * octantTransform.Xy;
                var gridY = startingPosition.Y + currentCol * octantTransform.Yx + yc * octantTransform.Yy;
                ...
                var leftBlockSlope = (yc + 0.5f) / (currentCol - 0.5f);
                var rightBlockSlope = (yc - 0.5f) / (currentCol + 0.5f);
                ...
                float distanceSquared = currentCol * currentCol + yc * yc;
                
                var isWall = grid.IsWall(gridX, gridY);
                var cornerBlocked = IsCornerBlocked(grid, gridX, gridY, startingPosition.X, startingPosition.Y);
                var currentBlocked = isWall || cornerBlocked;

                if (distanceSquared <= viewRadiusSq && cornerBlocked is false)
                {
                    grid.SetLight(gridX, gridY);
                }
                ...
                if (prevWasBlocked) 
                {
                    ...
                    else 
                    {
                        ...
                        // view area to the right corner of the last wall we saw. Also 
                        // prevent slope from reopening wider than before.
                        prevWasBlocked = false;
                        leftViewSlope = Math.Min(savedRightSlope, leftViewSlope);
                    }
                } 
                else 
                {
                    if (currentBlocked)
                    {
                        ...
                        {
                            CastLight(grid, startingPosition, viewRadius, currentCol + 1,
                                leftViewSlope, leftBlockSlope, octantTransform);
                        }
    ...
    private static bool IsCornerBlocked(IShadowCastGrid grid, int x, int y, int originX, int originY)
    {
        var dx = x - originX;
        var dy = y - originY;
        ...
        return grid.IsWall(x - stepX, y) 
               && grid.IsWall(x, y - stepY);
    }
}